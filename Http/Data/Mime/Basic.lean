...
inductive GeneralType.Standard
  ...
  | message
  | multipart
  deriving Repr, FromString

instance : ToString GeneralType.Standard where
  toString
  | .text => "text"
  | .image => "image"
  | .audio => "audio"
...
instance : ToString GeneralType where
  toString
    | .standard val => toString val
    | .custom val => toString val
...
instance : ToString Mime where
  toString mime :=
    let paramsStr := mime.params.foldl (fun acc k v => acc ++ "; " ++ toString k ++ "=" ++ v) ""
    toString mime.type ++ "/" ++ toString mime.subType ++ paramsStr
...
instance [ToString α] : ToString (Any α) where
  toString
    | .any => "*"
    | .specific r => toString r
...
instance : ToString MediaRange where
  toString mr :=
    let paramsStr := mr.params.foldl (fun acc k v => acc ++ ", " ++ toString k ++ "=" ++ v) ""
    toString mr.type ++ "/" ++ toString mr.subType ++ paramsStr
...
* Reference: https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types
-/
inductive Mime.Class
  | discrete
  | multipart

/-- Gets the type of a media type. -/
def GeneralType.Standard.defineType : GeneralType.Standard → Mime.Class
